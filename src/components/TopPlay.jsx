    ...
    <div className='flex-1 flex flex-row justify-between items-center'>
      ...
      <div className='flex-1 flex flex-col justify-center mx-3'>
        <Link to={`/song/${song.key}`}>
          <p className='text-xl font-bold text-gray-200 hover:text-white'>{song?.title}</p>
        </Link>
        <Link to={`/artists/${song?.artists[0].adamid}`}>
          <p className='text-ase  text-gray-300 mt-1 hover:text-white'>{song?.subtitle}</p>
        </Link>
      </div>
    </div>
...
const TopPlay = () => {
  ...
  const handlePlayClick = (song, i) => {
    dispatch(setActiveSong({ song, data, i })), dispatch(playPause(true))
  }
  ...
  return (
    <div
      ref={divRef}
      className='xl:ml-6 ml-0 xl:mb-6 flex-1 xl:max-w-[450px] max-w-full flex flex-col'
    >
